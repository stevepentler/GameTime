...
function advanceRound() {
  $gameStatus.text('Nice catch! On to the next round'); 
}
...
function playerWins() {
  $gameStatus.html('You win! <h3>musta been born on the bayou!</h3> <h3>Press Enter to play again!</h3>');
  sessionStorage.setItem(winsCounter, boat.score);
  var wins = sessionStorage;
  for (var key in wins) {
    $winningScores.prepend().html("<p>" + "Previous Score:" + wins[key] + "</p>");
  }
...
function playerLoses() {
  clearScoreAndRound();
  $gameStatus.html('Shrimpin\' Ain\'t Easy! <h3>Press Enter to play again!');
  resetRound();
}
