...
function xCollision(fish, bullet) {
  fish.x <= bullet.x && (bullet.x + bullet.width) <= (fish.x + fish.width);
}

function yCollision(fish, bullet) {
  if (bullet.y > 25) {
    fish.y <= bullet.y && bullet.y <= (fish.y + fish.height);
  }
}
...
function noCollisions(fishLocation) {
  return;
}
